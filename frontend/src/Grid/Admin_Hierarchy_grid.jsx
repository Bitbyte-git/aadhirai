...
function showChainPopup(anchorEl, ancestors, current, dark, pinned = false) {
  ...
  if (!document.getElementById('chain-popup-styles')) {
    ...
    s.textContent = `
      ...
      #chain-popup::-webkit-scrollbar-thumb:hover{background:linear-gradient(180deg,#4ade80,#38bdf8)}
  ...
  const itemsHtml = chain.map((item, idx) => {
    ...
    const iconKey = ICON_BY_TYPE[item.type] || 'shield'
...
function LaneCard({ node, role, active, onClick, ancestors, dark, text, subtext, onMessage, onPrint, activeStatusFilter, onToggleStatusFilter }) {
  ...
  const childCount = childRole
    ? (node.child_count !== undefined
        ? node.child_count
        : (role === 'admin' ? (node.dealer_count ?? 0) : (node[CHILD_KEY[role]] || []).length))
    : null
  ...
  return (
    <div className="glane">
      <div className="glane-label" style={{ '--nc': cfg.color }}>
        <span className="glane-level">LEVEL {LEVEL_NUM[role] || 5}</span>
...
export default function Admin_Hierarchy_grid() {
    ...
    ;(root.dealers || []).forEach(dealer => {
      ...
      ;(dealer.sub_dealers || []).forEach(sd => {
        ...
        ;(sd.promotors || []).forEach(pr => {
          ...
          searchCustomers(pr.customers || [], [{ node: root, role: 'admin' }, { node: dealer, role: 'dealer' }, { node: sd, role: 'sub_dealer' }, { node: pr, role: 'promotor' }])
        })
      })
    })
    return result
  ...
  const jumpToSearchResult = (item) => {
    ...
    item.ancestors.forEach(a => {
      if (a.role !== 'admin' && a.role !== 'customer') map[a.role] = a.node.id
      ...
      <div className="sahg-page" style={{ minHeight: '100vh', background: '#FFFFFF', color: text, fontFamily: '"Inter",system-ui,sans-serif', padding: '28px 32px' }}>
        ...
        <div className="sahg-header" style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 20, flexWrap: 'wrap', gap: 16 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            ...
            <button onClick={() => navigate('/admin')} style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(220,38,38,0.35)', color: '#DC2626', borderRadius: '10px', padding: '9px 16px', cursor: 'pointer', fontSize: '13px', fontWeight: 700 }}>
          ...
          {loading && (
            <div style={{ display: 'flex', gap: '14px', overflowX: 'auto', paddingBottom: '10px' }}>
              {Array.from({ length: 5 }).map((_, i) => (
                <SkeletonCard key={i} color={ROLE_CFG.dealer.color} />
              ))}
            </div>
          )}
