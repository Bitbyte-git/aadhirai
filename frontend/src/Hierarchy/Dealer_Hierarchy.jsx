...
function printHorizontalBracketTree(rootNode, role, ancestors, dealerInfo) {
...
function showChainPopup(anchorEl, ancestors, current, dark, text, subtext, dealerInfo, pinned = false) {
...
function TreeNode({ node, role, depth = 0, dark, text, subtext, ancestors = [], dealerInfo = {}, flatMode = false, parentKey = null, openMap = {}, onToggle = () => {}, onPrint = () => {}, childrenCache = {}, loadingNode = null }) {
      ...
      >
        ...
        <button
          onClick={e => {
            ...
            showChainPopup(e.currentTarget.closest('.otree-card'), ancestors, { node, role }, dark, text, subtext, dealerInfo, true)
          }}
          ...
          <button
            onClick={e => {
              ...
              if (CHILD_ROLE[role]) {
                ...
                printPersonCard(node, role, cfg, c, ancestors, dealerInfo)
              }
            }}
...
export default function DealerHierarchy() {
  ...
  const handlePrintOnly = () => {
    ...
    printPersonCard(node, role, cfg, color, ancestors, dealerInfo)
    setPrintTarget(null)
  }
  const handlePrintHierarchy = () => {
    const { node, role, ancestors } = printTarget
    printHorizontalBracketTree(node, role, ancestors, dealerInfo)
    setPrintTarget(null)
  }
  ...
  }, [subDealers, filter, debouncedSearch, openMap, treeZoom])
  ...
  const fetchHierarchy = async () => {
    ...
    try {
      ...
      if (res.data?.root) {
        setDealerInfo({
          dealer_id: res.data.root.dealer_id,
  ...
  const totalStats = subDealers ? {
    ...
    promotors: subDealers.reduce((a, sd) => a + (sd.promotors?.length || 0), 0),
    customers: subDealers.reduce((a, sd) => a + (sd.promotors || []).reduce((b, pr) => b + (pr.customers?.length || 0), 0), 0),
  } : null
  ...
  return (
    <div className="sh-page-wrap">
      <style>{`
        ...
        .sh-superadmin-line{ width:2px; flex:1; background:${ROLE_CFG.dealer.color}; margin-top:6px; }
      ...
      <div className="sh-topbar">
        ...
        <div className="sh-controls">
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, width: '100%', justifyContent: 'space-between', flexWrap: 'wrap' }}>
            ...
            <button onClick={() => navigate('/dealer')} style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(220,38,38,0.35)', color: '#DC2626', borderRadius: '10px', padding: '9px 16px', cursor: 'pointer', fontSize: '13px', fontWeight: 700, flexShrink: 0 }}>
      ...
      <div ref={treeWrapperRef} className="sh-canvas">
        ...
        {loading && (
          <div style={{ display: 'flex', gap: '16px', overflowX: 'auto', paddingBottom: '10px' }}>
            <SkeletonCard color={ROLE_CFG.sub_dealer.color} />
            <SkeletonCard color={ROLE_CFG.sub_dealer.color} />
            <SkeletonCard color={ROLE_CFG.sub_dealer.color} />
          </div>
        )}
            ...
            return (
              <div style={{ display: 'flex', gap: '20px', flexWrap: 'wrap', justifyContent: 'center', padding: '0 32px' }}>
                {filteredResults.map((item, idx) => (
                  <TreeNode key={item.node.id || idx} node={item.node} role={item.role} dark={dark} text={text} subtext={subtext} ancestors={item.ancestors} dealerInfo={dealerInfo} flatMode={true} onPrint={openPrintPopup} />
                ))}
              </div>
            )
          })() : filter ? (() => {
                ...
                {flatList.length === 0 ? (
                  ...
                  <div style={{ display: 'flex', gap: '20px', flexWrap: 'wrap', justifyContent: 'center' }}>
                    {flatList.map((item, idx) => (
                      <TreeNode key={item.node.id || idx} node={item.node} role={filter} dark={dark} text={text} subtext={subtext} ancestors={item.ancestors} dealerInfo={dealerInfo} flatMode={true} onPrint={openPrintPopup} />
                    ))}
                  </div>
                )}
              </div>
