...
function printPersonCard(node, role, cfg, color, ancestors, promotorInfo) {
  const chain = [
...
function printHorizontalBracketTree(rootNode, role, ancestors, promotorInfo) {
...
function showChainPopup(anchorEl, ancestors, current, dark, text, subtext, promotorInfo, pinned = false) {
      ...
      >
        ...
        <button
          onClick={e => {
            ...
            showChainPopup(e.currentTarget.closest('.otree-card'), ancestors, { node, role }, dark, text, subtext, promotorInfo, true)
          }}
        ...
        <div className="otree-actions">
          <button
            onClick={e => {
              e.stopPropagation()
              printPersonCard(node, role, cfg, c, ancestors, promotorInfo)
            }}
...
export default function PromotorHierarchy() {
  ...
  const handlePrintOnly = () => {
    ...
    printPersonCard(node, role, cfg, color, ancestors, promotorInfo)
    setPrintTarget(null)
  }
  const handlePrintHierarchy = () => {
    const { node, role, ancestors } = printTarget
    printHorizontalBracketTree(node, role, ancestors, promotorInfo)
    setPrintTarget(null)
  }
  ...
  useLayoutEffect(() => {
    ...
    const measure = () => {
      ...
      const cusEls = wrapper.querySelectorAll('[data-role="customer"]')
      const anchors = Array.from(cusEls).map(el => {
  ...
  }, [customers, debouncedSearch, treeZoom])
  ...
  return (
    <div className="sh-page-wrap">
      <style>{`
        ...
        .sh-superadmin-line{ width:2px; flex:1; background:${ROLE_CFG.promotor.color}; margin-top:6px; }
      ...
      <div className="sh-topbar">
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <button
              onClick={() => navigate('/promotor-hierarchy-grid')}
          ...
          {customers && (
            <div style={{ display: 'flex', gap: '10px', marginTop: '14px', flexWrap: 'wrap' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '6px', background: `${ROLE_CFG.customer.color}22`, border: `1px solid ${ROLE_CFG.customer.color}55`, borderRadius: '20px', padding: '4px 14px' }}>
                ...
                <span style={{ color: subtext, fontSize: '12px', fontWeight: 650 }}>Customers</span>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="sh-controls">
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, width: '100%', justifyContent: 'space-between', flexWrap: 'wrap' }}>
            ...
            <button onClick={() => navigate('/promotor')} style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(220,38,38,0.35)', color: '#DC2626', borderRadius: '10px', padding: '9px 16px', cursor: 'pointer', fontSize: '13px', fontWeight: 700, flexShrink: 0 }}>
          ...
          debouncedSearch ? (() => {
            ...
            return (
              <div style={{ display: 'flex', gap: '20px', flexWrap: 'wrap', justifyContent: 'center', padding: '0 32px' }}>
                {searchResults.map((cus, idx) => (
                  <TreeNode key={cus.id || idx} node={cus} role="customer" dark={dark} text={text} subtext={subtext} ancestors={[]} promotorInfo={promotorInfo} flatMode={true} onPrint={openPrintPopup} />
                ))}
              </div>
            )
          })() : customers.length === 0 ? (
            ...
            <div ref={scrollAreaRef} className="sh-tree-scroll">
              <div style={{ transformOrigin: 'top left', transform: 'scale(' + treeZoom + ')' }}>
                <div className="otree-children otree-children-root" style={{ '--lc': ROLE_CFG.customer.color, minWidth: 'max-content' }}>
                  {customers.map(cus => (
                    <div className="otree-item" key={cus.id} style={{ paddingTop: 0 }}>
                      <TreeNode
