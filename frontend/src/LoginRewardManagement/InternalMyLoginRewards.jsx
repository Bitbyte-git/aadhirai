...
const styles = `
  ...
  .lrt-tx-head{display:grid;grid-template-columns:1.4fr 1fr 90px 110px;gap:0;padding:12px 8px;border-bottom:1px solid rgba(189,207,206,.72)}
  .lrt-tx-head span{font-size:10px;font-weight:800;color:${MUTED};letter-spacing:.8px;text-transform:uppercase}
  .lrt-tx-row{display:grid;grid-template-columns:1.4fr 1fr 90px 110px;gap:0;align-items:center;padding:13px 8px;border-bottom:1px solid rgba(189,207,206,.4)}
...
export default function InternalMyLoginRewards() {
  ...
  const fetchData = async (p = 1, period = activeFilter, from = customFrom, to = customTo) => {
    ...
    try {
      ...
      let url = `/login-reward-transactions/?scope=my&page=${p}&period=${period}`
  ...
  const downloadReport = async () => {
    ...
    try {
      const { default: api } = await import('../api')
      let url = `/login-reward-transactions/?scope=my&period=${activeFilter}&export=csv`
      if (activeFilter === 'custom' && customFrom && customTo) url += `&start_date=${customFrom}&end_date=${customTo}`
      const res = await api.get(url, { responseType: 'blob' })
      const blob = new Blob([res.data], { type: 'application/pdf' })
      const link = document.createElement('a')
      link.href = URL.createObjectURL(blob)
      link.download = `my-login-rewards-${activeFilter}.pdf`
    ...
    <div className="lrt-page">
      ...
      <main className="lrt-main">
        <div className="lrt-headrow">
          <div>
            <p className="lrt-kicker">{ROLE_LABELS[role] || 'My Account'}</p>
            <h1 className="lrt-title">My Login Rewards</h1>
            <p className="lrt-note">Your own daily &amp; streak login-reward AUG Coins, plus coins manually sent to you — combined into one ledger.</p>
          </div>
        ...
        {loading ? (
          ...
          <>
            ...
            <section className="lrt-panel">
              <h3 className="lrt-panel-title">My Transactions {activeFilter !== 'custom' ? `— ${FILTERS.find(f => f.key === activeFilter)?.label}` : ''} ({summary.total_transactions})</h3>
              {rows.length === 0 ? (
                ...
                <>
                  ...
                  {rows.map((r, i) => (
                    <div key={i} className="lrt-tx-row">
                      ...
                      <div><span className="lrt-tx-date">{fmtDate(r.date)}</span></div>
                      <div></div>
                    </div>
                  ))}
                </>
