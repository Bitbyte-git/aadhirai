...
const styles = `
  ...
  .lrt-role-row{display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin-bottom:16px}
  .lrt-role-tab{padding:11px 20px;border-radius:12px;border:1.5px solid #D1DFDE;background:#fff;color:${DARK};font-weight:800;font-size:12.5px;cursor:pointer;transition:.15s ease;white-space:nowrap}
  .lrt-role-tab.active{border-color:${PRIMARY};background:linear-gradient(135deg,${PRIMARY},${DEEP});color:#fff;box-shadow:0 10px 24px rgba(7,59,63,.22)}
  ...
  @media(max-width:768px){
    ...
    .lrt-role-row,.lrt-filter-row{overflow-x:auto!important;-webkit-overflow-scrolling:touch!important;flex-wrap:nowrap!important;padding-bottom:8px!important}
...
export default function LoginRewardTransaction() {
  ...
  const fetchData = async (r = role, p = 1, period = activeFilter, from = customFrom, to = customTo) => {
    ...
    try {
      ...
      let url = `/login-reward-transactions/?role=${r}&page=${p}&period=${period}`
  ...
  useEffect(() => { fetchData('all', 1, 'today', '', '') }, [])
  ...
  const downloadReport = async () => {
    ...
    try {
      const { default: api } = await import('../api')
      let url = `/login-reward-transactions/?role=${role}&period=${activeFilter}&export=csv`
      if (activeFilter === 'custom' && customFrom && customTo) url += `&start_date=${customFrom}&end_date=${customTo}`
      const res = await api.get(url, { responseType: 'blob' })
      const blob = new Blob([res.data], { type: 'application/pdf' })
      const link = document.createElement('a')
      link.href = URL.createObjectURL(blob)
      link.download = `login-reward-transactions-${role}-${activeFilter}.pdf`
  ...
  const activeRole = ROLE_TABS.find(t => t.key === role)
    ...
    <div className="lrt-page">
      ...
      <main className="lrt-main">
        <div className="lrt-headrow">
          <div>
            <p className="lrt-kicker">Super Admin</p>
            <h1 className="lrt-title">Login Reward Transactions</h1>
            <p className="lrt-note">Daily &amp; streak login-reward AUG Coins, plus manually sent coins — combined into one ledger, per role and period.</p>
          </div>
        ...
        <div className="lrt-role-row">
          {ROLE_TABS.map(t => (
            <button
              key={t.key}
              type="button"
              className={`lrt-role-tab ${role === t.key ? 'active' : ''}`}
        ...
        {loading ? (
          ...
          <>
            ...
            <section className="lrt-panel">
              <h3 className="lrt-panel-title">{activeRole.label} Transactions {activeFilter !== 'custom' ? `— ${FILTERS.find(f => f.key === activeFilter)?.label}` : ''} ({summary.total_transactions})</h3>
              {rows.length === 0 ? (
