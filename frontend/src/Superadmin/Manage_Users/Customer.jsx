...
export default function Customer() {
  ...
  const fetchData = async (signal, currentOffset, searchTerm, append, retryCount = 0) => {
    ...
    try {
      const res = await api.get('/hierarchy/tier-directory/', {
        signal,
        params: { role: 'customer', offset: currentOffset, limit: PAGE_SIZE, search: searchTerm },
      })
  ...
  const downloadReport = async () => {
    ...
    setDownloading(true)
    try {
      const columns = ['S.No', 'Customer ID', 'Name', 'Email', 'Mobile', 'City']
      ...
      const res = await api.post(
        '/generic-table-pdf/',
        {
          ...
          subtitle: 'All Customers under this network, with today’s activity.',
      ...
      link.href = url
      link.download = `customers_${new Date().toISOString().slice(0, 10)}.pdf`
      ...
      <div className="mu-shell">
        ...
        <div className="mu-topbar">
          <div className="mu-breadcrumb">
            ...
            <span>/</span>
            <span style={{ color: '#073B3F', fontWeight: 700 }}>Customers</span>
          </div>
        ...
        <div className="mu-header-card">
          <div className="mu-header-info">
            <h1>
              ...
              <span className="mu-role-badge">Tier 1 • Base Network</span>
            </h1>
            <p className="mu-header-sub">
              Manage registered direct customers, downline referral performance, and member account directories.
            </p>
          </div>
        ...
        <div className="mu-stats-grid">
          <div className="mu-stat-card">
            <div>
              <span className="mu-stat-title">Total Registered</span>
          ...
          <div
            ...
            onClick={() => navigate('/login-active', { state: { roleFilter: 'Customer', viewMode: 'active', scopeLabel: 'Customers' } })}
            title="Open Active Users page filtered to Customers"
          >
            <div>
          ...
          <div
            className="mu-stat-card"
            style={{ cursor: 'pointer' }}
            onClick={() => navigate('/login-inactive', { state: { roleFilter: 'Customer', scopeLabel: 'Customers' } })}
            title="Open Inactive Users page filtered to Customers"
          >
            <div>
          ...
          <div
            className="mu-stat-card"
            style={{ cursor: 'pointer' }}
            onClick={() => navigate('/hierarchy-sales-count?role=customer&period=today')}
            title="Open today's order breakdown per Customer"
          >
            <div>
            ...
            <>
              <div className="mu-table-wrap">
                <table className="mu-table">
                  ...
                  <tbody>
                    {rows.map((a, i) => {
                      const initials = `${(a.first_name || 'U')[0]}${(a.last_name || '')[0] || ''}`.toUpperCase()
                      return (
                        <tr key={a.id || a.customer_id}>
                          <td className="mu-sno">{i + 1}</td>
                          <td>
                            <span
                              className="mu-id-pill"
                              onClick={() => handleCopyId(a.customer_id, `cid-${a.id}`)}
                          ...
                          <td className="mu-action-cell" onClick={(e) => e.stopPropagation()}>
                            ...
                            {actionOpen === a.id && (
                              <div className="mu-menu-popup">
                                ...
                                <button type="button" onClick={() => handleCopyId(a.customer_id, `cid-${a.id}`)}>
                                  <span>{copiedId === `cid-${a.id}` ? 'ID Copied!' : 'Copy Customer ID'}</span>
                                  <span>{copiedId === `cid-${a.id}` ? '✓' : '⧉'}</span>
                                </button>
                              </div>
                            )}
                          </td>
              ...
              <div className="mu-mobile-cards">
                {rows.map((a, i) => {
                  const initials = `${(a.first_name || 'U')[0]}${(a.last_name || '')[0] || ''}`.toUpperCase()
                  return (
                    <div key={`mob-${a.id || a.customer_id}`} className="mu-mobile-card">
                      <div className="mu-mobile-top">
                        ...
                        <span className="mu-id-pill" onClick={() => handleCopyId(a.customer_id, `cid-${a.id}`)}>
                          {a.customer_id}
                        </span>
                      </div>
                      ...
                      <div style={{ display: 'flex', gap: '8px', marginTop: '4px' }}>
                        ...
                        <button
                          type="button"
                          onClick={() => navigate(`/hierarchy-sales-count?role=customer&id=${a.id}`)}
              ...
              {hasMore && (
                <div className="mu-loadmore-wrap">
                  ...
                  >
                    {loadingMore ? 'Loading more members...' : `Load More (${rows.length} / ${totalCount})`}
                  </button>
                </div>
              )}
            </>
        ...
        >
          ...
          >
            ...
            <div style={{ padding: 24, display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: 12 }}>
              {[
                ['Customer ID', selectedDetail.customer_id],
