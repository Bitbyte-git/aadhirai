...
export default function Retailer() {
  ...
  const downloadReport = async () => {
    ...
    setDownloading(true)
    try {
      const columns = ['S.No', 'Retailer ID', 'Name', 'Email', 'Mobile', 'City']
      ...
      const statsPayload = [
        { label: 'Total Retailers', value: totalCount },
      ...
      const res = await api.post(
        '/generic-table-pdf/',
        {
          title: 'Retailer Directory Report',
          subtitle: 'All Retailers under this network, with today’s activity.',
      ...
      link.href = url
      link.download = `retailers_${new Date().toISOString().slice(0, 10)}.pdf`
