      ...
      <div className="mu-shell">
        ...
        <div className="mu-topbar">
          <div className="mu-breadcrumb">
            ...
            <span>/</span>
            <span style={{ color: '#073B3F', fontWeight: 700 }}>Super Stockists</span>
          </div>
        ...
        <div className="mu-header-card">
          <div className="mu-header-info">
            <h1>
              <span>Super Stockist Network</span>
              <span className="mu-role-badge">Tier 5 • Master Stockists</span>
            </h1>
            <p className="mu-header-sub">
              Manage enterprise super stockists, regional territory allocations, executive network oversight, and hierarchy trees.
            </p>
          </div>
            ...
            <>
              <div className="mu-table-wrap">
                <table className="mu-table">
                  ...
                  <tbody>
                    {rows.map((a, i) => {
                      const initials = `${(a.first_name || 'S')[0]}${(a.last_name || '')[0] || ''}`.toUpperCase()
                      return (
                        <tr key={a.id || a.admin_id}>
                          <td className="mu-sno">{i + 1}</td>
                          <td>
                            <span
                              className="mu-id-pill"
                              onClick={() => handleCopyId(a.admin_id, `aid-${a.id}`)}
                              title="Click to copy ID"
                            >
                              <span>{a.admin_id || '—'}</span>
                          ...
                          <td className="mu-action-cell" onClick={(e) => e.stopPropagation()}>
                            ...
                            {actionOpen === a.id && (
                              <div className="mu-menu-popup">
                                ...
                                <button type="button" onClick={() => handleCopyId(a.admin_id, `aid-${a.id}`)}>
                                  <span>{copiedId === `aid-${a.id}` ? 'ID Copied!' : 'Copy Admin ID'}</span>
                                  <span>{copiedId === `aid-${a.id}` ? '✓' : '⧉'}</span>
                                </button>
                              </div>
                            )}
                          </td>
              ...
              <div className="mu-mobile-cards">
                {rows.map((a, i) => {
                  const initials = `${(a.first_name || 'S')[0]}${(a.last_name || '')[0] || ''}`.toUpperCase()
                  return (
                    <div key={`mob-${a.id || a.admin_id}`} className="mu-mobile-card">
                      ...
                      <div style={{ display: 'flex', gap: '8px', marginTop: '4px' }}>
                        ...
                        <button
                          type="button"
                          onClick={() => navigate(`/hierarchy-sales-count?role=admin&id=${a.id}`)}
              ...
              {hasMore && (
                <div className="mu-loadmore-wrap">
                  ...
                  >
                    {loadingMore ? 'Loading more stockists...' : `Load More (${rows.length} / ${totalCount})`}
                  </button>
                </div>
              )}
            </>
        ...
        >
          ...
          >
            ...
            <div style={{ padding: 24, display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: 12 }}>
              {[
                ['Admin ID', selectedDetail.admin_id],
