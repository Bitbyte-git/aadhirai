...
export default function Wholesale_Dealer() {
  ...
  const fetchData = async (signal, currentOffset, searchTerm, append, retryCount = 0) => {
    ...
    try {
      const res = await api.get('/hierarchy/tier-directory/', {
        signal,
        params: { role: 'sub_dealer', offset: currentOffset, limit: PAGE_SIZE, search: searchTerm },
      })
  ...
  const downloadReport = async () => {
    ...
    setDownloading(true)
    try {
      const columns = ['S.No', 'Wholesale Dealer ID', 'Name', 'Email', 'Mobile', 'City']
      ...
      const statsPayload = [
        { label: 'Total Wholesale Dealers', value: totalCount },
      ...
      const res = await api.post(
        '/generic-table-pdf/',
        {
          title: 'Wholesale Dealer Directory Report',
          subtitle: 'All Wholesale Dealers under this network, with today’s activity.',
      ...
      link.href = url
      link.download = `wholesale_dealers_${new Date().toISOString().slice(0, 10)}.pdf`
      ...
      <div className="mu-shell">
        ...
        <div className="mu-topbar">
          <div className="mu-breadcrumb">
            ...
            <span>/</span>
            <span style={{ color: '#073B3F', fontWeight: 700 }}>Wholesale Dealers</span>
          </div>
        ...
        <div className="mu-header-card">
          <div className="mu-header-info">
            <h1>
              <span>Wholesale Dealer Network</span>
              <span className="mu-role-badge">Tier 3 • Sub-Dealers</span>
            </h1>
            <p className="mu-header-sub">
              Manage registered wholesale dealers, bulk volume distribution performance, and hierarchy trees.
            </p>
          </div>
        ...
        <div className="mu-stats-grid">
          <div className="mu-stat-card">
            <div>
              <span className="mu-stat-title">Total Wholesale Dealers</span>
          ...
          <div
            ...
            onClick={() => navigate('/login-active', { state: { roleFilter: 'Sub Dealer', viewMode: 'active', scopeLabel: 'Wholesale Dealers' } })}
            title="Open Active Users page filtered to Wholesale Dealers"
          >
            <div>
          ...
          <div
            className="mu-stat-card"
            style={{ cursor: 'pointer' }}
            onClick={() => navigate('/login-inactive', { state: { roleFilter: 'Sub Dealer', scopeLabel: 'Wholesale Dealers' } })}
            title="Open Inactive Users page filtered to Wholesale Dealers"
          >
            <div>
          ...
          <div
            className="mu-stat-card"
            style={{ cursor: 'pointer' }}
            onClick={() => navigate('/hierarchy-sales-count?role=sub_dealer&period=today')}
            title="Open today's order breakdown per Wholesale Dealer"
          >
            <div>
          ...
          {loading ? (
            ...
            <div style={{ padding: '60px 20px', textAlign: 'center' }}>
              ...
              <h4 style={{ margin: '0 0 6px', color: '#073B3F', fontSize: '16px', fontWeight: 800 }}>
                {search ? `No wholesale dealers match "${search}"` : 'No Wholesale Dealers Found'}
              </h4>
            ...
            <>
              <div className="mu-table-wrap">
                <table className="mu-table">
                  ...
                  <tbody>
                    {rows.map((a, i) => {
                      const initials = `${(a.first_name || 'W')[0]}${(a.last_name || '')[0] || ''}`.toUpperCase()
                      return (
                        <tr key={a.id || a.sub_dealer_id}>
                          <td className="mu-sno">{i + 1}</td>
                          <td>
                            <span
                              className="mu-id-pill"
                              onClick={() => handleCopyId(a.sub_dealer_id, `sdid-${a.id}`)}
                              title="Click to copy ID"
                            >
                              <span>{a.sub_dealer_id || '—'}</span>
                          ...
                          <td className="mu-action-cell" onClick={(e) => e.stopPropagation()}>
                            ...
                            {actionOpen === a.id && (
                              <div className="mu-menu-popup">
                                ...
                                <button type="button" onClick={() => handleCopyUrl(a, a.sub_dealer_id)}>
                                  <span>{copiedUrlId === a.id ? 'URL Copied!' : copyingUrl === a.id ? 'Copying…' : 'Copy Referral URL'}</span>
                                  <span>{copiedUrlId === a.id ? '✓' : '🔗'}</span>
                                </button>
                                <button type="button" onClick={() => handleCopyId(a.sub_dealer_id, `sdid-${a.id}`)}>
                                  <span>{copiedId === `sdid-${a.id}` ? 'ID Copied!' : 'Copy Dealer ID'}</span>
                                  <span>{copiedId === `sdid-${a.id}` ? '✓' : '⧉'}</span>
                                </button>
                              </div>
                            )}
                          </td>
              ...
              <div className="mu-mobile-cards">
                {rows.map((a, i) => {
                  const initials = `${(a.first_name || 'W')[0]}${(a.last_name || '')[0] || ''}`.toUpperCase()
                  return (
                    <div key={`mob-${a.id || a.sub_dealer_id}`} className="mu-mobile-card">
                      <div className="mu-mobile-top">
                        ...
                        <span className="mu-id-pill" onClick={() => handleCopyId(a.sub_dealer_id, `sdid-${a.id}`)}>
                          {a.sub_dealer_id}
                        </span>
                      </div>
                      ...
                      <div style={{ display: 'flex', gap: '8px', marginTop: '4px' }}>
                        ...
                        <button
                          type="button"
                          onClick={() => navigate(`/hierarchy-sales-count?role=sub_dealer&id=${a.id}`)}
              ...
              {hasMore && (
                <div className="mu-loadmore-wrap">
                  ...
                  >
                    {loadingMore ? 'Loading more dealers...' : `Load More (${rows.length} / ${totalCount})`}
                  </button>
                </div>
              )}
            </>
        ...
        >
          ...
          >
            ...
            <div style={{ padding: 24, display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: 12 }}>
              {[
                ['Wholesale Dealer ID', selectedDetail.sub_dealer_id],
